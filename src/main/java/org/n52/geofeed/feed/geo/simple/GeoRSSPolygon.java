...
import org.n52.geofeed.feed.geo.IPolygon;
 ...
 *
 */
public class GeoRSSPolygon extends BaseGeoFeedElement implements IPolygon{

    public GeoRSSPolygon(String name, String uri, Attributes attibutes) {
        super(name, uri, attibutes);
    }

    @Override
    protected void parseGeometry() throws MalformedGeoElementExecption {
        String[] coordStrings = getElement(POLYGON).getContentString().split(" ");
        if(coordStrings.length%2 != 0)
            throw new MalformedGeoElementExecption(POLYGON);
